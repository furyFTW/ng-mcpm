...
@Component({
  selector: 'ngmd-${component}-${demo}',
  template: require('./${component}-${demo}.html')
})
export class Ngmd${capitalize(component)}${capitalize(demo)} {
}
`;
}
...
function genDemosIndex(component) {
  ...
  const demoImports =
    demoNames
      .map((demo) => {
        return `import {Ngmd${capitalize(component)}${capitalize(demo)}} from './${demo}/${component}-${demo}';`;
      })
      .join('\n');

  const demoDirectives = demoNames.map((demo) => { return `Ngmd${capitalize(component)}${capitalize(demo)}`; });
